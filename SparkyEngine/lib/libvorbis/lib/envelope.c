...
static int _ve_amp(envelope_lookup *ve,
  ...
  int stretch=fmax(VE_MINSTRETCH,ve->stretch/2);
 ...
 /* window and transform */
  ...
  for(j=0;j<VE_BANDS;j++){
    ...
    /* convert amplitude to delta */
    {
      ...
      p--;
      if(p<0)p+=VE_AMP;
      postmax=fmax(acc,filters[j].ampbuf[p]);
      postmin=fmin(acc,filters[j].ampbuf[p]);
      
      for(i=0;i<stretch;i++){
	p--;
	if(p<0)p+=VE_AMP;
	premax=fmax(premax,filters[j].ampbuf[p]);
	premin=fmin(premin,filters[j].ampbuf[p]);
      }
